...
void welcome (){
    ...
    printf ("˚˚ Welcome to the Amethyst Accueil System ˚˚\n");
...
void WIPE (){
    system ("cls");//implements a clear screen
}
...
{//begins the menu function
    ...
    {//begins do...while loop
        ...
        {//begin of switch statement
            case 1:
                printf ("You've selected option 1: Manage Employees\n");//outputs the option the user had selected
                employf();//calls to the manage employee function
                break;

            case 2:
                printf ("You've selected option 2: Print Pay Slip\n");//outputs the option the user had selected
                PAYSLIP();//calls to the payslip function
                break;

            case 3:
                printf ("You've selected option 3: Print Report\n");//outputs the option the user had selected
                report();//calls to the report function
                break;

            case 0:
                goodbye();//calls to the goodbye function
                exit (0);
    ...
    return opt;//returns the value of the variable "opt"'
...
{//begin of payslip function
    ...
    //declaration and initialization of variables
    int i, hrs_wrk, ov_hrs, units_dis, empid;
    ...
    while (fscanf(file, "%d,%49[^,],%49[^,],%c,%d,%d\n", &EMP[t_emp].id, EMP[t_emp].fname, EMP[t_emp] .lname, &EMP[t_emp].sex, &EMP[t_emp].acc, &EMP[t_emp].trn) == 6) {
        if (EMP[t_emp].id == empid) { // If the ID matches the search ID, update the record

        printf("\nProcessing for Employee ID: %d\n", EMP[t_emp].id);//displays to the user the ID for which the payslip is being generated for
    ...
    system ("pause");//as the name suggests, pauses the system,act as a break
    WIPE ();
...
void employf( int *id, char *fname, char *lname, char *sex,int *acc, int *trn)
{
    ...
    do{//the menu2.0 function prompt resides here; it stems off of the first option being "Manage Employees"
        ...
        {
            case 1:
                printf ("Selected Option: 1...Display Employees\n");
            ...
            case 2:
                printf ("Selected Option 2: Update Employee\n");
                ...
                while (fscanf(file, "%d,%49[^,],%49[^,],%c,%d,%d\n", &EMP[t_emp].id, EMP[t_emp].fname, EMP[t_emp] .lname, &EMP[t_emp].sex, &EMP[t_emp].acc, &EMP[t_emp].trn) == 6) {
                    if (EMP[t_emp].id == search_num) { // If the ID matches the search ID, update the record

                        printf("Employee found:\n");
                        printf("ID: %d\nFirst Name: %s\nLast Name: %s\nSex: %c\nAccount Number: %d\nTRN: %d\n",
                            EMP[t_emp].id, EMP[t_emp].fname, EMP[t_emp].lname, EMP[t_emp].sex, EMP[t_emp].acc, EMP[t_emp].trn);

                        printf("Enter new first name of Employee:\n");
                        scanf(" %49s", EMP[t_emp].fname);
                        printf("Enter new last name for Employee:\n");
                        scanf(" %49s", EMP[t_emp].lname);
                        printf("Enter new sex of Employee:\n");
                        scanf(" %c", &EMP[t_emp].sex);
                        printf("Enter new Employee's Account Number:\n");
                        scanf("%d", &EMP[t_emp].acc);
                        printf("Enter New Employee's Tax Registration Number:\n");
                        scanf("%d", &EMP[t_emp].trn);
            ...
            case 3:
                printf ("Selected Option 3...Search for an Employee\n");
                ...
                while (fgets(line, sizeof(line), file) != NULL) {
                    sscanf(line, "%d,%49[^,],%49[^,],%c,%d,%d", &EMP[t_emp].id, EMP[t_emp].fname, EMP[t_emp].lname, &EMP[t_emp].sex, &EMP[t_emp].acc, &EMP[t_emp].trn);
                    if (EMP[t_emp].id == search_num) {
                        printf("Employee found:\n");
                        printf("ID: %d\nFirst Name: %s\nLast Name: %s\nSex: %c\nAccount Number: %d\nTRN: %d\n",
                               EMP[t_emp].id, EMP[t_emp].fname, EMP[t_emp].lname, EMP[t_emp].sex, EMP[t_emp].acc, EMP[t_emp].trn);
                        foundb = true;
            ...
            case 4:
                printf ("Selected Option 4...Delete Employee\n");
                ...
                // Loop through each record in the original file
                while (fscanf(file, "%d,%49[^,],%49[^,],%c,%d,%d\n", &EMP[t_emp].id, EMP[t_emp].fname, EMP[t_emp].lname, &EMP[t_emp].sex, &EMP[t_emp].acc, &EMP[t_emp].trn) == 6) {
                    if (EMP[t_emp].id == search_num) { // If the ID matches the search ID, skip writing this record to the temporary file
            ...
            case 5:
                printf ("Selected Option: 5...Add Employee\n");
            ...
            case 0:
                menu();//takes the user back to the menu function
                break;
